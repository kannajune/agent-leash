"""Configuration for steerd, read from environment variables.
...
HOST = _env("STEERD_HOST", "127.0.0.1")
PORT = int(_env("STEERD_PORT", "8787"))
...
DECISION_TIMEOUT = int(_env("STEERD_TIMEOUT", "300"))

# What to do if no decision arrives before the timeout: "deny" (safe) or "allow".
ON_TIMEOUT = _env("STEERD_ON_TIMEOUT", "deny")
...
PUBLIC_URL = _env("STEERD_PUBLIC_URL", RELAY_URL)

# --- Notification backend (how the phone gets pinged) ---
# ntfy: set STEERD_NTFY_TOPIC (uses ntfy.sh unless NTFY_SERVER overrides).
NTFY_SERVER = _env("STEERD_NTFY_SERVER", "https://ntfy.sh")
NTFY_TOPIC = _env("STEERD_NTFY_TOPIC", "")

# Telegram: set both to enable.
TELEGRAM_BOT_TOKEN = _env("STEERD_TELEGRAM_BOT_TOKEN", "")
TELEGRAM_CHAT_ID = _env("STEERD_TELEGRAM_CHAT_ID", "")

# Tools that are always safe to auto-approve without pinging the phone.
# Comma-separated tool names, e.g. "Read,Glob,Grep".
AUTO_ALLOW_TOOLS = {
    t.strip() for t in _env("STEERD_AUTO_ALLOW", "Read,Glob,Grep,LS").split(",") if t.strip()
}
