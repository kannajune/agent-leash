...
approve everything blindly. **steerd** lets you hold the reins from your
...
> **Desktop app** spawns hooks with a *limited* `PATH`, so a bare `steerd-hook`
> can fail to resolve there. Find the real path with `which steerd-hook`
> (e.g. `~/.local/bin/steerd-hook` after a pipx install) and use that exact
